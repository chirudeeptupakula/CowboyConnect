...
from app.utils import get_current_user, get_user_from_header
...
def join_club(data: schemas.ClubJoin, db: Session = Depends(get_db), user: User = Depends(get_user_from_header)):
...
def log_timesheet(entry: schemas.TimesheetLog, db: Session = Depends(get_db), user: User = Depends(get_user_from_header)):
